...
2. Discover candidates in ego-lite, one publisher at a time, in this exact order: FT, WSJ, The Economist, then Bloomberg.
   ...
   - On Bloomberg, use this stability procedure for every `Load more` cycle:
     1. After opening Latest, wait for the page to finish loading and settle. Scroll until `Load more` is in the visible viewport, then re-observe the current page and confirm that the button exists and is enabled before clicking.
     2. Before every click, re-locate `Load more` from the latest page state; never reuse a stale element or ref after a rerender. Prefer a visible real pointer/interaction click through ego-lite rather than a hidden DOM trigger.
     3. Before clicking, record a baseline when available: the unique visible news-link count, the last visible item title, and another visible list marker. Click `Load more` once.
     4. After each click, wait up to 30 seconds and monitor those markers. Continue only after confirming at least one new visible entry (for example, a higher unique-link count, a changed last title, or a newly visible item); then return to step 2.
     5. If the click has no response or no new visible entry after the 30-second wait, stop at Bloomberg immediately. Do not refresh, retry, bypass, skip Bloomberg, continue to another source, or generate the dashboard. Hand off the same ego-lite task space to the user and ask them to operate `Load more` or restore the page; after explicit confirmation, take over that task space and resume from Bloomberg. Generate the dashboard only after at least one visible entry predates the window start and the boundary check is complete.
...
- If a Bloomberg `Load more` click produces no response or no new visible entries after the stability procedure's wait of up to 30 seconds, or the button disappears or becomes disabled before the listing reaches the window start, do not mark Bloomberg `部分完成`, skip it, continue to another source, or generate a partial dashboard. Hand off the same ego-lite task space and ask the user to operate `Load more` or restore the page; after explicit confirmation, take over that task space and resume from Bloomberg. If it still does not respond, remain paused and ask the user again. Generate the dashboard only after Bloomberg reaches the time-window boundary.
